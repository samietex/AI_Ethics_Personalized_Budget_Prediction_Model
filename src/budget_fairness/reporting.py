...
def main() -> None:
    ...
    p.add_argument(
        "--run-json", default="reports/artifacts/last_run.json", help="Path to last_run.json"
    )
    p.add_argument("--template", default="docs/model_card.md", help="Path to model card template")
    p.add_argument(
        "--out", default="docs/model_card.filled.md", help="Output path for filled model card"
    )
    args = p.parse_args()
